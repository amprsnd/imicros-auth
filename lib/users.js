...
module.exports = {
            ...
            handler(ctx) {
                ...
                return this.database.collection.findOne({ email: ctx.params.email })
                    ...
                    .then(() => {
                        ...
                        user.verified = false;
                        if (this.verifiedUsers.indexOf(user.email)>=0) {
                            this.logger.info("verified user created",{ verifiedUsers: this.verifiedUsers, user: user.email});
                            user.verified = true;
                        }
                        if (this.settings.disableVerification) user.verified = true;
    ...
    created() {
        ...
        this.verifiedUsers = _.get(this.settings, "verifiedUsers",[]);
